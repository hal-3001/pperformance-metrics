...
def returns(data,plot=True):
    rets=data.pct_change()
    if plot:
        rets.plot()
    return rets

def annualized_returns(returns,n_periods,plot=True):
    compound_growth=(1+returns).prod()
    length=returns.shape[0]
    if plot:
        (compound_growth**(n_periods/length)-1).plot.bar()

    return compound_growth**(n_periods/length)-1

def annualized_volatility(returns,n_periods,plot=True):
    if plot:
        (returns.std()*(n_periods**0.5)).plot.bar()
...
def semi_deviation(returns,portfolio=False):
    if portfolio:
        ...
        semi=returns[less_than_zero].cov()
    else:   
        less_than_zero=(returns<0)
...
def minimize_vol(target,ar,covar):
    ...
    weights=minimize(portfolio_volatility,args=(covar,), x0=initial_guess
...
def weights_msr(ar,covar,r_free_rate):
    initial_guess=[1/len(ar)]*len(ar)
    bounds=((0.0,1.0),)*len(ar)
    weights_equal_to_1={"type":"eq",
                       "fun":lambda weights:np.sum(weights)-1}
    
    def neg_sharpe(weights,ar,covar,r_free_rate):
        rets=portfolio_returns(ar,weights)
        vol=portfolio_volatility(weights,covar)
        return -(rets-r_free_rate/vol)

    weights=minimize(neg_sharpe,initial_guess,args=(ar,covar,r_free_rate),method="SLSQP"
    ,bounds=bounds,constraints=(weights_equal_to_1))
    
    return weights.x

def msr(ar,covar,r_free_rate):
    weights=weights_msr(ar,covar,r_free_rate)
    return portfolio_returns(ar,weights),portfolio_volatility(weights,covar)



def optimal_weights(ar,covar,n_points):
    target_returns=np.linspace(ar.min(),ar.max(),n_points)
    weights=[minimize_vol(target_return,ar,covar) for target_return in target_returns]
    return weights

def plot_optimal_portfolio(ar,covar,n_points):
    weights=optimal_weights(ar,covar,n_points)
    rets=[portfolio_returns(ar,weight) for weight in weights]
    risk=[portfolio_volatility(weight,covar) for weight in weights]
